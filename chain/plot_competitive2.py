...
def main():
    ...
    if not all_margins and not all_dem_vote_shares: # Check if any data was processed
    ...
    print(f"Calculated metrics for {len(all_margins)} districts across all partitionings.")

    # Plot 1: Histogram of Margins
    if all_margins:
        plt.figure(figsize=(12, 7))
        plt.hist(all_margins, bins=NUM_BINS, color='skyblue', edgecolor='black', alpha=0.7)
        plt.title(f'Distribution of Partisan Margins ({len(all_margins)} districts from {num_partitionings} partitionings)', fontsize=14)
        plt.xlabel('Partisan Margin ((Dem Votes - Rep Votes) / Total Votes)\n(Negative = Rep Leaning, Positive = Dem Leaning)', fontsize=12)
        ...
        # plt.show() # Call plt.show() once after all plots if needed, or not at all in non-interactive script
    else:
        print("No margin data to plot.")

    # Plot 2: Histogram of Democratic Vote Shares
    if all_dem_vote_shares:
        plt.figure(figsize=(12, 7))
        plt.hist(all_dem_vote_shares, bins=NUM_BINS, color='lightcoral', edgecolor='black', alpha=0.7)
        plt.title(f'Distribution of Democratic Vote Share ({len(all_dem_vote_shares)} districts from {num_partitionings} partitionings)', fontsize=14)
        ...
        print("No Democratic vote share data to plot.")

    print("\n--- Summary ---")
    if all_margins:
        mean_margin_val = np.mean(all_margins)
        std_margin_val = np.std(all_margins)
        median_margin_val = np.median(all_margins)
        print(f"Average Partisan Margin: {mean_margin_val:.3f} (Std: {std_margin_val:.3f})")
        print(f"Median Partisan Margin: {median_margin_val:.3f}")
        ...
        else: # Should not happen if all_margins is not empty, but good for robustness
            print(f"Number of districts with margin between -0.1 and 0.1 (approx. 45-55%): {competitive_districts_margin} (0.00%)")


    if all_dem_vote_shares:
        mean_share_val = np.mean(all_dem_vote_shares)
        std_share_val = np.std(all_dem_vote_shares)
        median_share_val = np.median(all_dem_vote_shares)
        print(f"Average Democratic Vote Share: {mean_share_val:.3f} (Std: {std_share_val:.3f})")
        print(f"Median Democratic Vote Share: {median_share_val:.3f}")
        ...
        else: # Should not happen if all_dem_vote_shares is not empty
            print(f"Number of districts with Dem vote share between 0.45 and 0.55: {competitive_districts_share} (0.00%)")


    if all_margins or all_dem_vote_shares:
        print(f"\nTo view the plots, open '{OUTPUT_FIGURE_MARGIN_FILE}' and/or '{OUTPUT_FIGURE_VOTE_SHARE_FILE}'.")
    else:
        print("\nNo data was processed, so no plots were generated.")
